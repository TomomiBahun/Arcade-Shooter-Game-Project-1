...
Marisa::Marisa(int speed, int power, float range)
{
	// speed = 8, power = 2, range = 11.0
	setCharacter(speed, power, range);
}

void Marisa::draw() const
{
	if (_noHitTimer == 0) {
		DrawRotaGraphF(_x, _y, 1.8f, 0.0f, Image::getIns()->getMarisa()[direction], TRUE);
